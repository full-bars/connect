...
// Site: multiClientWindow.resize,
// "if len(clients) < targetWindowSize && !isBackendDegraded() { expand }".
// Expansion requires BOTH room in the window and a healthy backend.
func TestGate_WindowExpandRequiresRoomAndHealthyBackend(t *testing.T) {
	resetBackendDegraded()
	defer resetBackendDegraded()

	expand := func(clients, target int) bool {
		return clients < target && !isBackendDegraded()
	}

	if !expand(2, 4) {
		t.Fatal("healthy backend with room must expand")
	}
	if expand(4, 4) {
		t.Fatal("must not expand when the window is already at target")
	}
	...
	if expand(2, 4) {
		t.Fatal("expanded while degraded: every added client needs its own contract")
	}

	noteBackendSuccess()
	if !expand(2, 4) {
		t.Fatal("did not resume expanding after the backend recovered")
	}
}
